...
from scanner import (
    ...
    parse_codex_jsonl_file, SOURCE_CLAUDE, SOURCE_CODEX,
...
def _make_codex_record(record_type, payload, timestamp="2026-08-30T10:00:00Z"):
    return json.dumps({
        "timestamp": timestamp,
        "type": record_type,
        "payload": payload,
...
def _make_codex_usage(input_tokens=100, output_tokens=50,
                      cached_input_tokens=10, cache_write_input_tokens=5,
                      reasoning_output_tokens=7):
    return _make_codex_record("event_msg", {
        "type": "token_count",
        "info": {
            "last_token_usage": {
                "input_tokens": input_tokens,
...
class TestCodexParsing(unittest.TestCase):
    ...
    def test_maps_token_count_usage_to_shared_shape(self):
        path = self._write_jsonl([
            _make_codex_record("session_meta", {
                "session_id": "codex-session",
                "cwd": "/home/user/project",
            }),
            _make_codex_record("turn_context", {
                "turn_id": "turn-1",
                "model": "gpt-5.6-sol",
            }),
            _make_codex_usage(),
        ...
        metas, turns, agents, line_count = parse_codex_jsonl_file(path)

        self.assertEqual(line_count, 3)
        self.assertEqual(len(metas), 1)
        self.assertEqual(len(agents), 0)
        self.assertEqual(metas[0]["source"], SOURCE_CODEX)
        self.assertEqual(metas[0]["session_id"], "codex-session")
        self.assertEqual(metas[0]["project_name"], "user/project")
        self.assertEqual(metas[0]["model"], "gpt-5.6-sol")
        self.assertEqual(len(turns), 1)
        self.assertEqual(turns[0]["source"], SOURCE_CODEX)
        self.assertEqual(turns[0]["input_tokens"], 100)
        self.assertEqual(turns[0]["output_tokens"], 50)
        self.assertEqual(turns[0]["cache_read_tokens"], 10)
        self.assertEqual(turns[0]["cache_creation_tokens"], 5)
        self.assertEqual(turns[0]["reasoning_output_tokens"], 7)
        self.assertTrue(turns[0]["source_record_id"].startswith("codex:"))

    def test_each_token_count_event_is_a_response(self):
        path = self._write_jsonl([
            _make_codex_record("session_meta", {
                "session_id": "codex-session", "cwd": "/tmp/project",
            }),
            _make_codex_record("turn_context", {
                "turn_id": "turn-1", "model": "gpt-5.6-terra",
            }),
            _make_codex_usage(input_tokens=100, output_tokens=25),
            _make_codex_usage(input_tokens=200, output_tokens=75,
                              reasoning_output_tokens=12),
        ])

        _, turns, _, _ = parse_codex_jsonl_file(path)

        self.assertEqual(len(turns), 2)
        self.assertEqual([turn["input_tokens"] for turn in turns], [100, 200])
        self.assertEqual(turns[-1]["output_tokens"], 75)
        self.assertEqual(turns[-1]["reasoning_output_tokens"], 12)

    def test_falls_back_to_rollout_id_when_session_id_is_absent(self):
        path = self._write_jsonl([
            _make_codex_record("session_meta", {"id": "rollout-1", "cwd": "/tmp/project"}),
            _make_codex_record("turn_context", {"model": "gpt-5.6-luna"}),
            _make_codex_usage(),
        ])

        metas, turns, _, _ = parse_codex_jsonl_file(path)

        self.assertEqual(metas[0]["session_id"], "rollout-1")
        self.assertTrue(turns[0]["source_record_id"].startswith("codex:"))

    def test_resolves_auto_review_to_billable_model(self):
        path = self._write_jsonl([
            _make_codex_record("session_meta", {"id": "review-rollout"}),
            _make_codex_record("turn_context", {"model": "codex-auto-review"}),
            _make_codex_usage(),
        ])

        metas, turns, _, _ = parse_codex_jsonl_file(path)

        self.assertEqual(metas[0]["model"], "gpt-5.5")
        self.assertEqual(turns[0]["model"], "gpt-5.5")

    def test_ignores_duplicate_cumulative_snapshot_and_uses_total_delta_fallback(self):
        first = {
        ...
        advanced_total = {
            ...
            "output_tokens": 50, "reasoning_output_tokens": 6,
            "total_tokens": 210,
        }
        path = self._write_jsonl([
            _make_codex_record("session_meta", {"session_id": "codex-session"}),
            _make_codex_record("turn_context", {"model": "gpt-5.5"}),
            _make_codex_record("event_msg", {"type": "token_count", "info": {
            ...
            _make_codex_record("event_msg", {"type": "token_count", "info": {
                "total_token_usage": advanced_total,
            }}),
        ])
        ...
        self.assertEqual(len(turns), 2)
        self.assertEqual([turn["output_tokens"] for turn in turns], [30, 20])
        self.assertEqual([turn["input_tokens"] for turn in turns], [100, 60])

    def test_skips_parent_prefix_replayed_into_spawned_rollout(self):
        path = self._write_jsonl([
            _make_codex_record("session_meta", {"session_id": "parent-thread"}),
            _make_codex_record("turn_context", {"model": "gpt-5.5"}),
            _make_codex_usage(input_tokens=100, output_tokens=20),
            _make_codex_usage(input_tokens=120, output_tokens=30),
            _make_codex_usage(input_tokens=140, output_tokens=40),
        ])
        prefix = [
            (100, 10, 5, 20, 7, 0),
            (120, 10, 5, 30, 7, 0),
        ...
        _, turns, _, _ = parse_codex_jsonl_file(path, replayed_usage_prefix=prefix)

        self.assertEqual(len(turns), 1)
        self.assertEqual(turns[0]["output_tokens"], 40)
...
class TestDatabaseOperations(unittest.TestCase):
    ...
    def test_init_db_migrates_legacy_sessions_without_losing_rows(self):
        legacy_path = Path(tempfile.mktemp(suffix=".db"))
        try:
            ...
            conn.execute("""
                INSERT INTO sessions
                    (session_id, project_name, first_timestamp, last_timestamp,
                     total_input_tokens, total_output_tokens, model, turn_count, topic)
                VALUES ('legacy-session', 'user/project', 't1', 't2', 10, 5,
                        'claude-sonnet-4-6', 1, 'Legacy topic')
            """)
            conn.commit()
            ...
            row = conn.execute(
                ...
                (SOURCE_CLAUDE, "legacy-session"),
            ).fetchone()
            columns = {r["name"] for r in conn.execute("PRAGMA table_info(sessions)")}
            conn.close()
            self.assertIsNotNone(row)
            self.assertEqual(row["total_input_tokens"], 10)
            self.assertEqual(row["topic"], "Legacy topic")
            self.assertIn("total_reasoning_output", columns)
        finally:
    ...
    def test_message_id_dedupe_is_scoped_to_source(self):
        turns = [{
            "source": SOURCE_CLAUDE, "session_id": "claude-session",
            "timestamp": "2026-08-30T10:00:00Z", "model": "claude-sonnet-4-6",
            "input_tokens": 100, "output_tokens": 50,
            "cache_read_tokens": 0, "cache_creation_tokens": 0,
            "tool_name": None, "cwd": "/tmp", "message_id": "shared-id",
        }, {
            "source": SOURCE_CODEX, "session_id": "codex-session",
            "timestamp": "2026-08-30T10:00:00Z", "model": "gpt-5.6-sol",
        ...
        count = self.conn.execute(
            "SELECT COUNT(*) FROM turns WHERE message_id = 'shared-id'"
        ).fetchone()[0]
...
class TestCodexScanIntegration(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.codex_dir = Path(self.tmpdir) / "sessions"
        self.codex_dir.mkdir()
        self.db_path = Path(self.tmpdir) / "usage.db"
        self.filepath = self.codex_dir / "rollout.jsonl"

    def _write_rollout(self, output_tokens=50):
        with open(self.filepath, "w") as f:
            f.write(_make_codex_record("session_meta", {
                "session_id": "same-id-as-claude",
                "cwd": "/home/user/codex-project",
            }) + "\n")
            f.write(_make_codex_record("turn_context", {
                "turn_id": "turn-1", "model": "gpt-5.6-sol",
            }) + "\n")
            f.write(_make_codex_usage(output_tokens=output_tokens) + "\n")

    def test_scan_stores_codex_source_and_reasoning(self):
        self._write_rollout()

        result = scan(codex_dir=self.codex_dir, db_path=self.db_path,
                      source=SOURCE_CODEX, verbose=False)

        self.assertEqual(result["turns"], 1)
        self.assertEqual(result["by_source"][SOURCE_CODEX]["new"], 1)
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        session = conn.execute(
            "SELECT * FROM sessions WHERE source = ? AND session_id = ?",
            (SOURCE_CODEX, "same-id-as-claude"),
        ).fetchone()
        turn = conn.execute(
            "SELECT * FROM turns WHERE source = ?",
            (SOURCE_CODEX,),
        ).fetchone()
        conn.close()
        self.assertIsNotNone(session)
        self.assertEqual(session["total_reasoning_output"], 7)
        self.assertEqual(turn["model"], "gpt-5.6-sol")

    def test_same_session_id_can_exist_for_both_sources(self):
        self._write_rollout()
        scan(codex_dir=self.codex_dir, db_path=self.db_path,
             source=SOURCE_CODEX, verbose=False)

        conn = get_db(self.db_path)
        init_db(conn)
        claude_session = {
            "source": SOURCE_CLAUDE, "session_id": "same-id-as-claude",
            "project_name": "user/claude-project",
            "first_timestamp": "2026-08-30T09:00:00Z",
            "last_timestamp": "2026-08-30T09:05:00Z", "git_branch": "main",
            "model": "claude-sonnet-4-6", "total_input_tokens": 10,
            "total_output_tokens": 5, "total_cache_read": 0,
            "total_cache_creation": 0, "turn_count": 1,
        }
        upsert_sessions(conn, [claude_session])
        conn.commit()
        rows = conn.execute(
            "SELECT source, session_id FROM sessions WHERE session_id = ? ORDER BY source",
            ("same-id-as-claude",),
        ).fetchall()
        conn.close()
        self.assertEqual([(r[0], r[1]) for r in rows], [
            (SOURCE_CLAUDE, "same-id-as-claude"),
            (SOURCE_CODEX, "same-id-as-claude"),
        ])

    def test_incremental_codex_scan_keeps_each_appended_token_event(self):
        self._write_rollout(output_tokens=50)
        scan(codex_dir=self.codex_dir, db_path=self.db_path,
             source=SOURCE_CODEX, verbose=False)

        with open(self.filepath, "a") as f:
            f.write(_make_codex_usage(input_tokens=150, output_tokens=75) + "\n")
        mtime = os.path.getmtime(self.filepath) + 1
        ...
        conn = sqlite3.connect(self.db_path)
        count, output = conn.execute(
            "SELECT COUNT(*), SUM(output_tokens) FROM turns WHERE source = ?",
            (SOURCE_CODEX,),
        ).fetchone()
        conn.close()
        self.assertEqual(count, 2)
    ...
    def test_codex_parser_revision_rebuilds_unchanged_rollouts(self):
        self._write_rollout(output_tokens=50)
        scan(codex_dir=self.codex_dir, db_path=self.db_path,
             source=SOURCE_CODEX, verbose=False)
        ...
        conn = get_db(self.db_path)
        conn.execute(
            "UPDATE turns SET output_tokens = 999 WHERE source = ?",
            (SOURCE_CODEX,),
        )
        _meta_set(conn, "codex_parser_revision", "old-parser")
        conn.commit()
        conn.close()

        # The JSONL mtime did not change.  The revision marker must still force
        # a one-time rebuild so corrected parsing reaches historic usage.
        scan(codex_dir=self.codex_dir, db_path=self.db_path,
             source=SOURCE_CODEX, verbose=False)
        ...
        conn.close()
        self.assertEqual((count, output), (1, 50))

    def test_scan_excludes_spawned_rollout_parent_replay(self):
        parent = self.codex_dir / "parent.jsonl"
        child = self.codex_dir / "child.jsonl"
        with open(parent, "w") as f:
            f.write(_make_codex_record("session_meta", {
                "id": "parent-rollout", "session_id": "parent-thread",
            }, "2026-08-30T10:00:00Z") + "\n")
            f.write(_make_codex_record("turn_context", {"model": "gpt-5.5"}) + "\n")
            f.write(_make_codex_usage(input_tokens=100, output_tokens=10) + "\n")
            f.write(_make_codex_usage(input_tokens=120, output_tokens=20) + "\n")
        with open(child, "w") as f:
            f.write(_make_codex_record("session_meta", {
                "id": "child-rollout", "session_id": "parent-thread",
                "forked_from_id": "parent-rollout",
            ...
            f.write(_make_codex_usage(input_tokens=100, output_tokens=10) + "\n")
            f.write(_make_codex_usage(input_tokens=120, output_tokens=20) + "\n")
            f.write(_make_codex_usage(input_tokens=140, output_tokens=30) + "\n")
...
class TestProjectNameAfterTitleRecord(unittest.TestCase):
    """A title record seeds the session meta but carries no cwd."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()

    def _write(self, lines):
        path = os.path.join(self.tmpdir, "titled.jsonl")
        with open(path, "w") as f:
            for line in lines:
                f.write(json.dumps(line) + "\n")
        return path

    def test_project_name_is_repaired_from_a_later_record(self):
        # custom-title lines can precede the first assistant record. Before the
        # fix the session was seeded "unknown" and never repaired — and
        # upsert_sessions never updates project_name, so it stayed unknown.
        path = self._write([
            {"type": "custom-title", "sessionId": "s1", "customTitle": "My topic"},
            {"type": "assistant", "sessionId": "s1",
             "timestamp": "2026-08-31T10:00:00Z",
             "cwd": "/Users/me/Projects/cool",
        ...
        self.assertEqual(len(metas), 1)
        self.assertEqual(metas[0]["project_name"], "Projects/cool")
        self.assertEqual(metas[0]["topic"], "My topic")
        self.assertEqual(len(turns), 1)

    def test_a_real_cwd_never_overwrites_another_real_cwd(self):
        # Only the "unknown" placeholder is repaired; the first real cwd wins,
        # so a resumed session in a different directory doesn't flip-flop.
        path = self._write([
            {"type": "assistant", "sessionId": "s1",
             "timestamp": "2026-08-31T10:00:00Z", "cwd": "/Users/me/first/dir",
             "message": {"id": "m1", "model": "claude-opus-5",
                         "usage": {"input_tokens": 10, "output_tokens": 5}}},
            {"type": "assistant", "sessionId": "s1",
             "timestamp": "2026-08-31T10:05:00Z", "cwd": "/Users/me/second/dir",
        ...
        self.assertEqual(metas[0]["project_name"], "first/dir")


class TestCodexIdentityEdgeCases(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()

    def _write_jsonl(self, lines, filename="rollout.jsonl"):
        path = os.path.join(self.tmpdir, filename)
        with open(path, "w") as f:
            for line in lines:
                f.write(line + "\n")
    ...
    def test_turn_context_does_not_relabel_a_spawned_rollout(self):
        # session_meta deliberately picks the immutable rollout id; the logical
        # thread id in turn_context is shared with the parent, so letting it win
        # collapsed child rollouts into their parent's session.
        path = self._write_jsonl([
            _make_codex_record("session_meta", {
                "id": "child-rollout",
            ...
            _make_codex_record("turn_context", {
                "session_id": "parent-thread", "model": "gpt-5.6-terra",
            }),
            _make_codex_usage(),
        ])
        ...
        self.assertEqual(metas[0]["session_id"], "child-rollout")
        self.assertEqual(turns[0]["session_id"], "child-rollout")

    def test_turn_context_still_supplies_a_missing_session_id(self):
        path = self._write_jsonl([
            _make_codex_record("turn_context", {
                "session_id": "only-source", "model": "gpt-5.6-luna",
            }),
            _make_codex_usage(),
        ])
        ...
        self.assertEqual(metas[0]["session_id"], "only-source")
        self.assertEqual(turns[0]["session_id"], "only-source")

    def test_identical_repeated_responses_both_survive(self):
        # Same session, timestamp, model and usage: a content-only record id
        # made the second one a duplicate and INSERT OR IGNORE dropped it.
        path = self._write_jsonl([
            _make_codex_record("session_meta", {"id": "rollout-1", "cwd": "/tmp/p"}),
            _make_codex_record("turn_context", {"model": "gpt-5.6-terra"}),
            _make_codex_usage(input_tokens=100, output_tokens=25),
            _make_codex_usage(input_tokens=100, output_tokens=25),
        ])
        ...
        self.assertEqual(len(turns), 2)
        self.assertEqual(len({turn["source_record_id"] for turn in turns}), 2)

    def test_record_ids_are_stable_across_an_incremental_reparse(self):
        # The occurrence counter advances for historical lines too, so an
        # appended event gets the same id a full reparse would give it.
        lines = [
            _make_codex_record("session_meta", {"id": "rollout-1", "cwd": "/tmp/p"}),
            _make_codex_record("turn_context", {"model": "gpt-5.6-terra"}),
            _make_codex_usage(input_tokens=100, output_tokens=25),
        ...
        _, full, _, _ = parse_codex_jsonl_file(path)
        _, appended, _, _ = parse_codex_jsonl_file(path, start_line=3)

        full_ids = [turn["source_record_id"] for turn in full]
        self.assertEqual(len(appended), 1)
        self.assertIn(appended[0]["source_record_id"], full_ids)
        self.assertEqual(appended[0]["source_record_id"], full_ids[-1])
